...
import { IOfferModel } from "@/services/interfaces/common";
...
export async function getServerSideProps(
  ...
  const offerData = await OfferApi.index(slug as string);
...
export default function Store({ data }: { data: IOfferModel }) {
  return (
    <div style={{ position: "relative", minHeight: "100vh" }}>
      <Head>
        <title>{`Finderr - ${data.title} at ${data.brand?.name} Jaipur`}</title>
      ...
      <div className="pb-20">
        ...
        <div className="w-full md:w-10/12 max-w-[1400px] flex flex-col lg:flex-row gap-8 p-2 mx-auto mb-8 mt-2">
          ...
          <div className="flex-1">
            <p className="text-2xl font-bold py-2 md:py-4">{data.title}</p>
